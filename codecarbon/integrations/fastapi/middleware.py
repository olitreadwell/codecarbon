...
class CodeCarbonMiddleware:
    ...
    def _finalize_on_worker(
        ...
        # Bound tracker._tasks: the record is persisted, keeping it would leak
        # one Task per request for the lifetime of the server.
        discard = getattr(tracker, "discard_task", None)
        if callable(discard):
            discard(resolved_task)
        self._record_totals(detection, task_name, emissions_data)
        measurement = self._build_measurement(
            detection, task_name, resolved_task, emissions_data, tracker
        )
        try:
        ...
        if run_callback:
            self._run_request_complete(
                request, response, measurement.emissions_data, resolved_task
            )
        return measurement
